...
class Fun(commands.Cog, name="Fun"):
	...
	@commands.command()
	async def spank(self, ctx, *, tget:discord.Member):
			titles = ['Reeeeeeee',
	        	'BOP!',
	        	'Get beamed',
	        	'dang',
	        	...
	        	'that outa hurt ;-;']
			img = nekos.img("spank")
			embed = discord.Embed(title=f"{random.choice(titles)}", description=f"{ctx.author.mention} **spanked** {tget.mention} **silly**.", colour=0xC0C0C0)
			embed.set_image(url=img)
			await ctx.send(embed=embed)
